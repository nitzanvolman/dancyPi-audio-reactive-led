...
def visualize_solid(y):
    """Effect that originates in the center and scrolls outwards"""
    global p

    # print(config.N_PIXELS)
    # print(param.children()[2].value().red())

    px2 = int(config.N_PIXELS / 2)

    p[0, :] = [param.children()[2].value().red()] * px2
    p[1, :] = [param.children()[2].value().green()] * px2
    p[2, :] = [param.children()[2].value().blue()] * px2
...
prevVisType = "spectrum"
visualization_effect = visualize_spectrum
def processInput():
    global visualization_effect, prevVisType
    # print(param.children()[1].value())
    visType = param.children()[1].value()
    if visType != prevVisType:
        prevVisType = visType
        if visType == "spectrum":
            visualization_effect = visualize_spectrum
        elif visType == "energy":
            ...
            visualization_effect = visualize_solid
        else:
            visualization_effect = visualize_spectrum
...
if config.USE_GUI:
    ...
    # effect_selection = QtGui.QComboBox(items={
    #     'energy' : 'energy',
    #     'scroll' : 'scroll'
    # })
    # app.addItem(effect_selection)
    # layout.nextRow()
